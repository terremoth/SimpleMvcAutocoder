...
function class_vars($sClassName) {
...
//		$aValue = array($sWhereFieldValue);
    ...
    public function getAllObjects() 
    {
        ...
        $aResults = $this->get(strtolower($calledClass));
        $aObjs = array();
        
        foreach ($aResults as $aRow) {
            
            $oObj = new $calledClass();
            
            foreach ($aRow as $column => $value) {                
                $method = ucfirst($column);
                $oObj->{"set$method"}($value);
            }
    ...
    {
        return $this->insert(strtolower(get_called_class()), $post);
    }
}
